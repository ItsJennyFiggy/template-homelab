...
*   **Static Metadata Registry**: To avoid wasting tokens on API listing calls (e.g. querying workspace members, projects, or states), refer directly to the static registry at [workspace-metadata.md](.agents/skills/plane-ticket-formatter/references/workspace-metadata.md) to locate workspace, project, state, and user UUIDs.
